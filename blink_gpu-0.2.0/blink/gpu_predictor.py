...
class GPUPredictor:
    ...
    def __init__(self, model_path=None, memory_model_path=None, cache_size=100):
        ...
        self.feature_cols = [
            ...
            'num_conv_layers', 'num_fc_layers', 'num_bn_layers',
            'avg_conv_kernel_size', 'max_conv_channels',
            'total_conv_params', 'total_fc_params',
        ...
        if features_to_predict:
            ...
            for features in features_to_predict:
                ...
                for col in self.feature_cols:
                    if col == 'batch_size':
                        default_val = 1
                    elif col == 'tflops_fp32':
                        default_val = 12.0
                    elif col == 'memory_bandwidth_gbps':
                        default_val = 336.0
                    elif col == 'sm_count':
                        default_val = 30.0
                    elif col == 'vocab_size':
                        ...
                        default_val = 1
                    elif col == 'kv_cache_size_mb':
                        default_val = 0.0
                    elif col == 'quantization_bits':
                        default_val = 32
                    else:
                        default_val = 0
            ...
            # Update results and cache ensuring quantile non-crossing
            for i, pred_idx in enumerate(cache_indices):
                # Ensure q0.1 <= q0.5 <= q0.9
                ...
                pred_dict = {
                    'exec_time_ms': median,
                    'exec_time_lower': lower,
                    'exec_time_upper': upper,
    ...
    def predict_for_custom_model(self, model, batch_size):
        ...
        if getattr(self, 'use_gnn', False) and isinstance(model, nn.Module):
            try:
                ...
                # --- Real quantile CI bounds (Fix #3) ---
                # Use the already-loaded execution_lower / execution_upper XGBoost
                # quantile models (q0.1 / q0.9) on a tabular feature vector so
                # the GNN path participates in the same calibration framework as
                # the XGBoost path. Fall back to ±10% only if feature extraction
                # throws.
                exec_lower = exec_time * 0.9  # fallback
                exec_upper = exec_time * 1.1  # fallback

                if self.has_exec_bounds:
                    try:
                        _extractor = ModelFeatureExtractor(save_dir='data/temp')
                        model.to("cpu")
                        _feats = _extractor.extract_model_features(model, (3, 224, 224))
                        model.to(device)
                        _feats['batch_size'] = batch_size

                        _defaults = {
                            'batch_size': 1, 'tflops_fp32': 12.0,
                            'memory_bandwidth_gbps': 336.0, 'sm_count': 30.0,
                            'vocab_size': 0, 'seq_len': 1,
                            'kv_cache_size_mb': 0.0, 'quantization_bits': 32,
                        }
                        _row = {
                            col: _feats.get(col, _defaults.get(col, 0))
                            for col in self.feature_cols
                        }
                        _df = pd.DataFrame([_row])[self.feature_cols]

                        q10 = float(np.expm1(self.model_lower.predict(_df)[0]))
                        q90 = float(np.expm1(self.model_upper.predict(_df)[0]))
                        exec_lower = min(q10, exec_time)   # q0.1 ≤ median
                        exec_upper = max(q90, exec_time)   # q0.9 ≥ median
                    except Exception as _ci_err:
                        print(f"GNN CI quantile extraction failed, using ±10% fallback: {_ci_err}")
