...
def load_model_data():
    data_files = []
    for file in os.listdir(os.path.join(BASE_DIR, 'data', 'raw')):
            ...
            if pasted_code:
                try:
                    ...
                    sandbox = {'nn': nn, 'torch': torch, 'models': models,
                               'F': F, 'np': np, 'math': __import__('math')}
            ...
            if model is None:
                ...
                with st.spinner("Analyzing model architecture..."):
                    ...
                    if model_type == "PyTorch Code (GNN)":
                        # Route through GNN predictor substitute
                        for bs in batch_sizes:
                            gnn_res = predictor.predict_for_custom_model(model, bs)
                            exec_lower = gnn_res.get('exec_lower_ms', gnn_res.get('exec_time_lower', 0))
                            exec_upper = gnn_res.get('exec_upper_ms', gnn_res.get('exec_time_upper', 0))
                            exec_str = f"{gnn_res['exec_time_ms']:.2f} [{exec_lower:.2f} - {exec_upper:.2f}]"
                            
                            mem_usage = gnn_res['memory_usage_mb']
                        ...
                        for i, bs in enumerate(batch_sizes):
                            ...
                            exec_str = f"{pred_data['exec_time_ms']:.2f} [{pred_data.get('exec_time_lower', 0):.2f} - {pred_data.get('exec_time_upper', 0):.2f}]" 
                ...
                st.write(f"Total Parameters: {features.get('total_parameters', 0):,}")
    ...
    with col2:
        ...
        **Steps:**
        ...
        if 'features' in locals():
            ...
            patterns = features.get('architecture_patterns', {})
            
            pattern_data = {
                "Pattern": ["Skip Connections", "Attention Mechanism", "Normalization Layers", "Model Depth"],
                "Present": [
                    "âœ“" if patterns.get('has_skip_connections', False) else "âœ—",
                    "âœ“" if patterns.get('has_attention', False) else "âœ—",
                    "âœ“" if patterns.get('has_normalization', False) else "âœ—",
                    str(patterns.get('max_depth', False))
                ]
            }
